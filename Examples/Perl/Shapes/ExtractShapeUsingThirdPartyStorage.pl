...
$response = $slidesApi->GetSlidesSlideShapes(name => $name, slideIndex=>$slideIndex, storage=>$storage);

if($response->{'Status'} eq 'OK'){
	foreach my $shapeLink (@{$response->{'ShapeList'}->{'ShapesLinks'}}){
    print "\nShape Href :: " . $shapeLink->{'Uri'}->{'Href'};
	}	
}
#ExEnd:1